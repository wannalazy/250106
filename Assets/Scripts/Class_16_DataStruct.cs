...
using static WRX.Class_15.Class_15_Generics;

namespace WRX.Class_16
{
    /// <summary>
    /// 資料結構
    /// </summary>
    public class Class_16_DataStruct : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                numbers.Add(i);
                LogSystem.LogWithColor($"數量 : {numbers.Count}", "#f39");
            ...
            LogStack<string>(enemys);
            // 判斷資料是否包含某筆資料
            LogSystem.LogWithColor($"{enemys.Contains("哥布林")}", "#3f3");
            // 清除所有資料
            enemys.Clear();
            LogStack<string>(enemys);
            #endregion
            ...
            // 如果資料不需要頻繁增減建議使用 SortedList 反之建議使用 SortedDictionary
        }
